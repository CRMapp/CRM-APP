...
static NSString *_key		= @"a16byteslongkey!a16byteslongkey!";
CCOptions padding	= kCCOptionPKCS7Padding;

+ (NSString *) EncryptString:(NSString *)plainSourceStringToEncrypt withKey:(NSString*)encryKey
{
	...
	NSData *encryptedData		=	[crypto encrypt:_secretData key:[encryKey dataUsingEncoding:NSUTF8StringEncoding] padding:&padding];
	return   [Base64 encode:encryptedData];
	
}


+ (NSString *)DecryptString:(NSString *)base64StringToDecrypt withKey:(NSString*)decryKey
{
	StringEncryption *crypto	= [[[StringEncryption alloc] init] autorelease];
	NSData *data				= [crypto decrypt:[Base64 decode:base64StringToDecrypt] key:[decryKey dataUsingEncoding:NSUTF8StringEncoding] padding: &padding];
...
{
	StringEncryption *crypto	= [[[StringEncryption alloc] init] autorelease];
	NSData *data				= [crypto decrypt:[Base64 decode:base64StringToDecrypt] key:[_key dataUsingEncoding:NSUTF8StringEncoding] padding: &padding];
...
//    } 
   ...
   //LOGGING_FACILITY1( ccStatus == kCCSuccess, @"Problem with CCCryptorUpdate, ccStatus == %d.", ccStatus );
	...
	if(plainTextBufferSize>totalBytesWritten && encryptOrDecrypt>0)
	{
	cipherOrPlainText=	[self doCipherWithoutPading:plainText key:aSymmetricKey context:encryptOrDecrypt padding:pkcs7];
	}
...
- (NSData *)doCipherWithoutPading:(NSData *)plainText
				 key:(NSData *)aSymmetricKey
			 context:(CCOperation)encryptOrDecrypt
			 padding:(CCOptions *)pkcs7
{
